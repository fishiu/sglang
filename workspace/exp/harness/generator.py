...
MODEL_MAPPING = {
    'qw_mha_1_5b': '/iopsstor/scratch/cscs/xjin/cache/hf_home/hub/models--Qwen--Qwen1.5-4B-Chat/snapshots/a7a4d4945d28bac955554c9abd2f74a71ebbf22f',
...
def main():
    ...
    parser.add_argument('--model', required=True, help='Comma separated list of model keys (qw_mha_1_5b, qw_4b, qw_30b_th, qw_30b)')
    parser.add_argument('--method', required=True, choices=['base', 'quest', 'stream'], help='Method to use (base, quest, stream)')
    ...
    parser.add_argument('--wind', default=None, help='streaming_llm_window_length options (int list or range). REQUIRED for stream.')
    parser.add_argument('--sink', default=None, help='streaming_llm_num_sink_tokens options (int list or range). REQUIRED for stream.')
    ...
    if args.method != 'quest':
        if args.topk != '16' or args.est != 'triton':
             print("Error: --topk and --est are only allowed when --method=quest")
             sys.exit(1)

    if args.method == 'stream':
        if args.wind is None or args.sink is None:
            print("Error: --wind and --sink are required when --method=stream")
            sys.exit(1)
    elif args.wind is not None or args.sink is not None:
         print("Error: --wind and --sink are only allowed when --method=stream")
         sys.exit(1)
    ...
    keys = ['model', 'pg', 'graph', 'frac', 'limit', 'bsz', 'ctx', 'tasks', 'run']
    iterables = [models, pgs, graphs, fracs, limits, bszs, ctxs, tasks, runs]
